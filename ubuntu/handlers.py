...
async def cancel(call: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(chat_id, text, reply_markup=admin_panel_markup)
    if call.from_user.id == admin_id:
        bot.send_message(chat_id,
                ("Это сообщение может увидеть только администратор.\n"
                "Сейчас в базе {count_users} человек!\n").format(count_uesrs=count_users), reply_markup=admin_markup)
...
@dp.mallback_query_handler(text_contains="hats")
...
@dp.mcallback_query_handler(text_contains="accessories")
async def show_items(message: Message):
    # Достаем товары из базы данных
...
@dp.mcallback_query_handler(text_contains="malling")
async def show_items(message: Message):
    # Достаем товары из базы данных
...
@dp.mcallback_query_handler(text_contains="pants")
async def show_items(message: Message):
    # Достаем товары из базы данных
...
@dp.mcallback_query_handler(text_contains="shoes")
async def show_items(message: Message):
    # Достаем товары из базы данных
