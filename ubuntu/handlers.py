...
async def basket_list(call: CallbackQuery, state: FSMContext):
    ...
    for num, product in enumerate(product_id):
        all_items = await database.Item.query.where(database.Item.id == product.product_id).gino.all()
        markup_2 = InlineKeyboardMarkup(
        inline_keyboard =
        [
            [InlineKeyboardButton(text=("Назад"), callback_data="cancel")],
        ...
        if not all_items:
            await call.message.answer("Корзина пуста.", reply_markup=markup_2)
            return
...
async def basket_delete(call: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(text, reply_markup=button)
    await database.Basket.delete.where(database.Basket.product_id == product_id and database.Basket.user_id == call.from_user.id).gino.status()
    await states.List_item.Next.set()
